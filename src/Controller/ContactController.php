...
class ContactController extends AbstractController
{
    ...
    public function index(Request $request, EntityManagerInterface $entityManager, MailerInterface $mailer): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!empty($missingFields)) {
                ...
                $mail = new Mailer();
                $mail->setName(trim($formData['name']));
                $mail->setPhone(trim($formData['phone']));
                $mail->setEmail(trim($formData['email']));
                $mail->setDescription(trim($formData['description']));
    ...
    private function createEmail(string $fromEmail, Mailer $mail, Subject $subject): Email
    {
        return (new Email())
            ...
            ->html($this->renderView('contact/EmailContact.twig', [
                'mailer' => $mail,
                'subject' => $subject
            ]));
    }
